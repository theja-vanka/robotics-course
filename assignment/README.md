...
57 skeletons, one per coding block. Each **loads a specific model + an open-source dataset** (e.g. LeRobot ALOHA joint states / scene images, a small torch tensor), splits the task into **functions** you fill, and ships **pytest tests** so running `pytest <file>` tells you pass/fail.
...
| Day01 | 🔧 Build + Milvus | First Collection | `Day01_first_collection.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day02 | 🛠️ Hands-on | Run LLaVA on Robot Scenes | `Day02_run_llava_on_robot_scenes.py` | `transformers datasets torch pillow` |
| Day02 | 🔧 Build + Milvus | Store CLIP Embeddings | `Day02_store_clip_embeddings.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day03 | 🔧 Build + Milvus | Index Types | `Day03_index_types.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day04 | 🔧 Build + Milvus | DINOv2 Object Store | `Day04_dinov2_object_store.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day05 | 🛠️ Hands-on | Train 3DGS with Nerfstudio | `Day05_train_3dgs_with_nerfstudio.py` | `nerfstudio` |
| Day05 | 🔧 Build + Milvus | Production Schema Design | `Day05_production_schema_design.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day05 | 🗜️ Compression | Pruning Hands-on | `Day05_pruning_hands_on.py` | `torch torchvision datasets pillow` |
| Day06 | 🔧 Build + Milvus | Scene Retrieval Service | `Day06_scene_retrieval_service.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day07 | 🛠️ Hands-on | Fine-tune YOLOv8 on YCB Dataset | `Day07_fine_tune_yolov8_on_ycb_dataset.py` | `ultralytics pytest` |
| Day07 | 🔧 Build + Milvus | Detection Embedding Store | `Day07_detection_embedding_store.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day08 | 🔧 Build + Milvus | Multi-modal (RGB + Depth) | `Day08_multi_modal_rgb_depth.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day09 | 🔧 Build + Milvus | Object Pose Library | `Day09_object_pose_library.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day10 | 🔧 Build + Milvus | Grasp Pose Library | `Day10_grasp_pose_library.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day11 | 🔧 Build + Milvus | Semantic SLAM Map | `Day11_semantic_slam_map.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day11 | 🗜️ Compression | TensorRT INT8 | `Day11_tensorrt_int8.py` | `onnxruntime torch torchvision datasets numpy pillow` |
| Day12 | 🔧 Build + Milvus | Pipeline + Milvus Integration | `Day12_pipeline_milvus_integration.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day13 | 🔧 Build + Milvus | Retrieval-Augmented VLA | `Day13_retrieval_augmented_vla.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day14 | 🔧 Build + Milvus | Demo Replay Buffer | `Day14_demo_replay_buffer.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day15 | 🔧 Build + Milvus | Demo-Episode Deduplication | `Day15_demo_episode_deduplication.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day15 | 🗜️ Compression | Low-rank Factorisation | `Day15_low_rank_factorisation.py` | `torch` |
| Day16 | 🔧 Build + Milvus | Demo-Episode Browser | `Day16_demo_episode_browser.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day17 | 🔧 Build + Milvus | Diversity-guided Augmentation | `Day17_diversity_guided_augmentation.py` | `pymilvus numpy datasets transformers torch pillow` |
| Day18 | 🛠️ Hands-on | Run DreamerV3 | `Day18_run_dreamerv3.py` | `# clone DreamerV3 (see resources)` |
| Day18 | 🔧 Build + Milvus | World Model State Memory | `Day18_world_model_state_memory.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day19 | 🔧 Build + Milvus | 3DGS Object Index | `Day19_3dgs_object_index.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day21 | 🔧 Build + Milvus | ANN Benchmarking | `Day21_ann_benchmarking.py` | `pymilvus numpy datasets transformers torch pillow` |
...
| Day23 | 🔧 Build + Milvus | Anomaly Detection | `Day23_anomaly_detection.py` | `pymilvus numpy datasets transformers torch pillow` |
