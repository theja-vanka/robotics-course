...
IMAGE = load_dataset("huggingface/cats-image", split="test")[0]["image"].convert(
    "RGB"
)  # a REAL sample image
...
def encode_image(model, processor):
    ...
    # 👇 write your code here, then DELETE the line below
    encode_input = processor(images=IMAGE, return_tensors="pt").to(DEVICE)
            ...
            "content": [
                {
                    "type": "text",
                    "text": """
                        List unique objects visible in this image.
                        Rules:
