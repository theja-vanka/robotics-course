...
**Step 3.** Get the starter file **`Day22_add_latency_profiler.py`** into Colab: click the **📁 folder icon** (left sidebar) → **Upload**, and pick it from your `intensive study/starter_code` folder. *(It's pre-written — a specific model + an open-source dataset are loaded for you; you only fill the functions.)*
...
**Step 5.** Run the tests — new cell, **▶**:  `!pytest Day22_add_latency_profiler.py -q`  →  the **red / FAILED** tests are your to-do list (each one names the function to write).
**Step 6.** Double-click **`Day22_add_latency_profiler.py`** in the 📁 panel to edit it. Fill in each function and DELETE its `raise` line, then re-run `!pytest Day22_add_latency_profiler.py -q` until **everything is green**. That completes: Add per-stage timing to the Day-20 pipeline; log to CSV; find the bottleneck. Deliverable: a CSV of per-stage ms + the slowest stage named + one optimization tried (before/after).
