# Day 12 — Mock Interview #1 + Pipeline Integration

> **Phase:** 🤖 Phase 2 — Robotics Perception Stack  ·  **📅 Week 2 · Sat**
...
| 9:30–11:00 | 🧠 **New Concept** | 🤖 Mock interview #1 | - [ ] Done |
| 11:00–11:15 | ☕ Break | — | — |
| 11:15–13:00 | 🛠️ **Hands-on** | 🤖 apply it → capstone | - [ ] Done |
| 13:00–14:00 | 🍽️ Lunch | Step away from screens | — |
| 14:00–15:30 | 🔧 **Build + Milvus** | 🗄️ Pipeline integration | - [ ] Done |
| 15:30–15:45 | ☕ Break | — | — |
| 15:45–17:15 | 🗜️ **Compression** | 🗜️ Benchmark table | - [ ] Done |
| 17:15–18:30 | ⚙️ C++ Literacy | Clone a robotics C++ repo & change one thing | - [ ] Done |
| 18:30–19:00 | 🍵 Dinner | Step away | — |
| 19:00–20:30 | 🧩 **DSA** | Mixed review set #1 | - [ ] Done |
...
**🎯 Today's focus:**
- 🤖 AI/Robotics: Mock interview #1
- 🗄️ Milvus *(supporting tool)*: Pipeline integration
- 🗜️ Compression: Benchmark table
- ⚙️ C++ *(literacy — read & modify)*: Clone a robotics C++ repo & change one thing
- 🧩 DSA: Mixed review set #1
...
**⏱️ Estimated resource time today:** ~230 min (~3.8 hr)

> 🌴 **Tomorrow is your rest day (Sunday).** No study — let memory consolidate off-screen.

---

## 🌅 Warm-up | 9:00–9:30 | Review All
**Task:** Speed-run: one flashcard per topic from Days 1-11.
...
---
## 🧠 New Concept | 9:30–11:00 | Mock Interview — Theory (90 min)
**Task:** Ask Claude to quiz you: VLMs, diffusion, 3DGS, detection, pose, grasp, SLAM, Milvus basics.

**🎯 Outcome:** A written artifact (summary + from-memory sketch) that proves you understand Mock Interview — Theory (90 min).

**▶️ DO THIS — every step (read in browser → write in Obsidian):**

**Step 1.** Click the first resource below (**Interview Prep Guide — Robotics CV**) — it opens in your browser.
**Step 2.** Read it for ~15 min. You don't need every word — get the big picture and the main diagram.
**Step 3.** In Obsidian, make a new note (click the 📄 **New note** icon, name it `Day 12 — notes`).
**Step 4.** With the article **closed**, write 5–8 sentences explaining **Mock Interview — Theory (90 min)** in your own words, and sketch the main flow.
...
**✅ Done when:**
- [ ] You can explain Mock Interview — Theory (90 min) aloud in ~2 min with notes closed
...
---
## 🛠️ Hands-on | 11:15–13:00 | Fix Gaps
**Task:** Identify 3 weakest topics from mock. Deep-dive each for 40 min.
...
---
## 🔧 Build + Milvus | 14:00–15:30 | Pipeline + Milvus Integration
...
**Step 3.** Get the starter file **`Day12_pipeline_milvus_integration.py`** into Colab: click the **📁 folder icon** (left sidebar) → **Upload**, and pick it from your `intensive study/starter_code` folder. *(It's pre-written — a specific model + an open-source dataset are loaded for you; you only fill the functions.)*
...
**Step 5.** Run the tests — new cell, **▶**:  `!pytest Day12_pipeline_milvus_integration.py -q`  →  the **red / FAILED** tests are your to-do list (each one names the function to write).
**Step 6.** Double-click **`Day12_pipeline_milvus_integration.py`** in the 📁 panel to edit it. Fill in each function and DELETE its `raise` line, then re-run `!pytest Day12_pipeline_milvus_integration.py -q` until **everything is green**. That completes: Wire all pipeline stages into main.py. Every detection/pose result upserted into Milvus. Smoke test.
...
---
## 🗜️ Compression | 15:45–17:15 | Benchmark Table
...
**Step 1.** In your browser, open **https://colab.research.google.com** → click **New notebook**. *(First time? Read [[Setup_Guide]] once — it explains every tool below.)*
**Step 2.** Turn on the free GPU: top menu **Runtime → Change runtime type → T4 GPU → Save**.
**Step 3.** Get the starter file **`Day12_benchmark_table.py`** into Colab: click the **📁 folder icon** (left sidebar) → **Upload**, and pick it from your `intensive study/starter_code` folder. *(It's pre-written — a specific model + an open-source dataset are loaded for you; you only fill the functions.)*
**Step 4.** Install its tools — new cell, **▶** (the file's `Setup:` line lists the exact ones):
...
**Step 5.** Run the tests — new cell, **▶**:  `!pytest Day12_benchmark_table.py -q`  →  the **red / FAILED** tests are your to-do list (each one names the function to write).
**Step 6.** Double-click **`Day12_benchmark_table.py`** in the 📁 panel to edit it. Fill in each function and DELETE its `raise` line, then re-run `!pytest Day12_benchmark_table.py -q` until **everything is green**. That completes: FP32 vs FP16 vs INT8 for YOLOv8. Create a table: model size, latency, mAP. Plot latency vs mAP.
   ❌ `ModuleNotFoundError: X` → new cell, run `!pip install X`, then retry.  ❌ `CUDA out of memory` → **Runtime → Disconnect and delete runtime**, reconnect with GPU, use the smaller model in `src/config.py`.  ❌ Anything else for >90 min → write down where you're stuck and move on (that's not 'behind').
**Step 7.** All tests green? Tick the ✅ boxes below and check **Block complete**.
...
---
## ⚙️ C++ Literacy | 17:15–18:30 | Clone a robotics C++ repo & change one thing
**Task:** Read & understand: clone a robotics c++ repo & change one thing. The goal is to READ and MODIFY, not write from scratch — trace what the code does, change one thing, rebuild/run, predict the effect.

**🎯 Outcome:** You can read and modify C++ Literacy — Clone a robotics C++ repo & change one thing — explain it, not reproduce it from scratch.

**▶️ DO THIS — every step (browser + online C++ compiler):**

**Step 1.** Click the first resource (**ROS2 examples (real C++ nodes)**) and read the example C++. This is **read & modify** — you are NOT writing it from scratch.
...
**Step 3.** Do it: Read & understand: clone a robotics c++ repo & change one thing. The goal is to READ and MODIFY, not write from scratch — trace what the code does, change one thing, rebuild/run, predict the effect.
...
---
## 🧩 DSA | 19:00–20:30 | Mixed review set #1
**Task:** Name the pattern, then solve 3–5 Mixed review set #1 problems. Test edge cases, state time/space complexity, and queue the hardest to re-solve from memory tomorrow.

**🎯 Outcome:** A few DSA — Mixed review set #1 problems solved cold, with the pattern named.
...
**Step 2.** Pick **3–5** problems on today's pattern (**Mixed review set #1**).
