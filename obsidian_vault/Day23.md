# Day 23 — Coding Interview Prep

> **Phase:** 🎯 Phase 4 — Interview Mastery + Second Product  ·  **📅 Week 4 · Fri**
...
| 9:30–11:00 | 🧠 **New Concept** | 🤖 Coding interview prep | - [ ] Done |
| 11:00–11:15 | ☕ Break | — | — |
| 11:15–13:00 | 🛠️ **Hands-on** | 🤖 apply it → capstone | - [ ] Done |
| 13:00–14:00 | 🍽️ Lunch | Step away from screens | — |
| 14:00–15:30 | 🔧 **Build + Milvus** | 🗄️ Anomaly detection | - [ ] Done |
| 15:30–15:45 | ☕ Break | — | — |
| 15:45–17:15 | 🗜️ **Compression** | 🗜️ Full benchmark report | - [ ] Done |
| 17:15–18:30 | 🔁 Capstone Lab | extra fine-tune / compress / eval reps | - [ ] Done |
| 18:30–19:00 | 🍵 Dinner | Step away | — |
| 19:00–20:30 | 🧩 **DSA** | Top-K & heap patterns | - [ ] Done |
...
**🎯 Today's focus:**
- 🤖 AI/Robotics: Coding interview prep
- 🗄️ Milvus *(supporting tool)*: Anomaly detection
- 🗜️ Compression: Full benchmark report
- 🔁 Capstone Lab *(reps on the capstone)*: extra fine-tune / compress / eval reps
- 🧩 DSA: Top-K & heap patterns
...
**⏱️ Estimated resource time today:** ~255 min (~4.2 hr)

---

## 🌅 Warm-up | 9:00–9:30 | Review
**Task:** Re-read Day 22 system design doc. Find 2 gaps.
...
---
## 🧠 New Concept | 9:30–11:00 | CV Coding Patterns
**Task:** Implement: IoU, NMS, mAP, bilinear interpolation, connected components.

**🎯 Outcome:** Working code plus saved output for CV Coding Patterns.
...
**Step 3.** In a Colab cell, install what you need, e.g. `!pip install torch transformers` (**▶**).
**Step 4.** New **+ Code** cell — write the code yourself, following the task + resources: Implement: IoU, NMS, mAP, bilinear interpolation, connected components.
**Step 5.** Press **▶**; check the output looks right.
...
---
## 🛠️ Hands-on | 11:15–13:00 | Custom PyTorch Dataset
...
**Step 1.** In your browser, open **https://colab.research.google.com** → click **New notebook**. *(First time? Read [[Setup_Guide]] once — it explains every tool below.)*
**Step 2.** Turn on the free GPU: top menu **Runtime → Change runtime type → T4 GPU → Save**.
**Step 3.** Get the starter file **`custom_pytorch_dataset.py`** into Colab: click the **📁 folder icon** (left sidebar) → **Upload**, and pick it from `intensive study/starter_code/Day23/`. *(It's pre-written — a specific model + an open-source dataset are loaded for you; you only fill the functions. Its shared setup lives in `starter_code/helpers/`.)*
...
---
## 🔧 Build + Milvus | 14:00–15:30 | Anomaly Detection
...
**Step 1.** In your browser, open **https://colab.research.google.com** → click **New notebook**. *(First time? Read [[Setup_Guide]] once — it explains every tool below.)*
**Step 2.** Turn on the free GPU: top menu **Runtime → Change runtime type → T4 GPU → Save**.
**Step 3.** Get the starter file **`anomaly_detection.py`** into Colab: click the **📁 folder icon** (left sidebar) → **Upload**, and pick it from `intensive study/starter_code/Day23/`. *(It's pre-written — a specific model + an open-source dataset are loaded for you; you only fill the functions. Its shared setup lives in `starter_code/helpers/`.)*
**Step 4.** Install its tools — new cell, **▶** (the file's `Setup:` line lists the exact ones):
...
---
## 🗜️ Compression | 15:45–17:15 | Full Benchmark Report
**Task:** Table: model size, FLOPs, latency, mAP for each compression technique applied so far.

**🎯 Outcome:** A written artifact (summary + from-memory sketch) that proves you understand Compression — Full Benchmark Report.

**▶️ DO THIS — every step (read in browser → write in Obsidian):**

**Step 1.** Click the first resource below (**PyTorch Model Analysis — torchinfo**) — it opens in your browser.
**Step 2.** Read it for ~15 min. You don't need every word — get the big picture and the main diagram.
**Step 3.** In Obsidian, make a new note (click the 📄 **New note** icon, name it `Day 23 — notes`).
**Step 4.** With the article **closed**, write 5–8 sentences explaining **Compression — Full Benchmark Report** in your own words, and sketch the main flow.
**Step 5.** Re-open the article, compare, fix mistakes, and write down **2 questions** you still have.
**Step 6.** Tick the ✅ boxes below and check **Block complete**.

**✅ Done when:**
- [ ] You can explain Compression — Full Benchmark Report aloud in ~2 min with notes closed
...
**Task:** Deliberate-practice reps on the capstone: `eval.py`: implement your metrics (success rate, IoU/NMS helpers) cleanly. Re-run it, change ONE variable (LoRA rank, precision, prompt, batch), measure the effect, and log a number to vla-edge/benchmarks/results.csv.
...
```
   ...
   ❌ `ModuleNotFoundError: X` → new cell, run `!pip install X`, then retry.  ❌ `CUDA out of memory` → **Runtime → Disconnect and delete runtime**, reconnect with GPU, use the smaller model in `src/config.py`.  ❌ Anything else for >90 min → write down where you're stuck and move on (that's not 'behind').
**Step 5.** Do the reps: Deliberate-practice reps on the capstone: `eval.py`: implement your metrics (success rate, IoU/NMS helpers) cleanly. Re-run it, change ONE variable (LoRA rank, precision, prompt, batch), measure the effect, and log a number to vla-edge/benchmarks/results.csv.
...
---
## 🧩 DSA | 19:00–20:30 | Top-K & heap patterns
**Task:** Name the pattern, then solve 3–5 Top-K & heap patterns problems. Test edge cases, state time/space complexity, and queue the hardest to re-solve from memory tomorrow.

**🎯 Outcome:** A few DSA — Top-K & heap patterns problems solved cold, with the pattern named.
...
**Step 2.** Pick **3–5** problems on today's pattern (**Top-K & heap patterns**).
